...
describe('End-to-end integration tests', () => {
  ...
  describe('value parameter semantics', () => {
    ...
    it('E2E: mutable parameter function compiles and runs correctly', async () => {
      const result = await compileFiles({
        '/project/main.efs': [
          'export let sortItems = (mut items: Array<number>): void => {',
          '  items.sort()',
          '}',
        ].join('\n'),
      });
      ...
      const js = getJS(result, 'main.js');
      expect(js).toBeDefined();
      // mut is erased — JS output is identical
      expect(js).toContain('const sortItems = (items) =>');
    });

    it('E2E: multi-file — imported function with mut parameter can be called', async () => {
      const result = await compileFiles({
        '/project/lib.efs': [
          'export let sortItems = (mut items: Array<number>): void => {',
          '  items.sort()',
    ...
    it('E2E: DTS output does not reflect mut', async () => {
      const result = await compileFiles({
        '/project/main.efs': [
          'export let sortItems = (mut items: Array<number>): void => {',
          '  items.sort()',
