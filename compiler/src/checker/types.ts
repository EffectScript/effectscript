...
/** A generic type parameter declaration (e.g. `T` in `<T>`). */
...
export function isAssignableTo(source: Type, target: Type, depth = 0): boolean {
  ...
  // Record structural subtyping (width + depth)
  if (s.kind === 'record' && t.kind === 'record') {
    for (const [key, targetFieldType] of t.fields) {
      const sourceFieldType = s.fields.get(key);
...
export function simplifyUnion(members: readonly Type[]): Type {
  ...
  if (unique.length >= 2 && unique.every(t => t.kind === 'adt')) {
    const adts = unique as ADTType[];
