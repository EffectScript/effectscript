 ...
 * deeply immutable by default; `mut` allows content mutation.
 */
...
describe('value parameter semantics — parser', () => {
  it('parses (mut x: number) — FunctionParam has mutable: true', () => {
    const prog = parseOk('let f = (mut x: number): number => x');
  ...
  it('parses mixed mutability: (mut items: Array<string>, name: string)', () => {
    const prog = parseOk('let f = (mut items: Array<string>, name: string): void => { }');
    const decl = prog.body[0] as LetDeclaration;
    const fn = decl.initializer as ArrowFunction;
  ...
  it('parses mut with default value: (mut x: number = 42)', () => {
    const prog = parseOk('let f = (mut x: number = 42): number => x');
    const decl = prog.body[0] as LetDeclaration;
    const fn = decl.initializer as ArrowFunction;
    expect(fn.params[0].mutable).toBe(true);
  ...
  it('rejects double mut: (mut mut x: number) — parse error', () => {
    const diagnostics = new DiagnosticCollectorImpl();
    const tokens = tokenize('let f = (mut mut x: number): number => x', 'test.efs', diagnostics);
    parse(tokens, 'test.efs', diagnostics);
    const errors = diagnostics.getAll().filter(d => d.severity === 'error');
    expect(errors.length).toBeGreaterThan(0);
  });

  it('rejects mut in type position: (x: mut number) — parse error', () => {
    const diagnostics = new DiagnosticCollectorImpl();
    const tokens = tokenize('let f = (x: mut number): number => x', 'test.efs', diagnostics);
    parse(tokens, 'test.efs', diagnostics);
    const errors = diagnostics.getAll().filter(d => d.severity === 'error');
    expect(errors.length).toBeGreaterThan(0);
  });

  it('mut in generic function: <T>(mut items: Array<T>): void', () => {
    const prog = parseOk('let f = <T>(mut items: Array<T>): void => { }');
    const decl = prog.body[0] as LetDeclaration;
    const fn = decl.initializer as ArrowFunction;
    expect(fn.params[0].mutable).toBe(true);
  });

  it('mut combined with return type annotation', () => {
    const prog = parseOk('let f = (mut items: Array<number>): Array<number> => items');
    const decl = prog.body[0] as LetDeclaration;
    const fn = decl.initializer as ArrowFunction;
    expect(fn.params[0].mutable).toBe(true);
  ...
  it('mut in extension function params', () => {
    const prog = parseOk('fun string.doSomething(mut items: Array<number>): void => { }');
...
describe('value parameter semantics — mutable params', () => {
  it('allows (mut items: Array<number>) => { items.push(42) }', () => {
    expectNoErrors(
      `let f = (mut items: Array<number>): void => { items.push(42) }`,
    );
  });

  it('allows (mut user: { name: string }) => { user.name = "Bob" }', () => {
    expectNoErrors(
      `let f = (mut user: { name: string }): void => { user.name = "Bob" }`,
    );
  });

  it('allows (mut items: Array<number>) => { items.sort() }', () => {
    expectNoErrors(
      `let f = (mut items: Array<number>): void => { items.sort() }`,
    );
  });

  it('rejects reassignment (mut items: Array<number>) => { items = [] } — E202', () => {
    expectErrors(
      `let f = (mut items: Array<number>): void => { items = [] }`,
      'E202',
    );
  });
  ...
  it('mixed: E240 on immutable, no error on mutable', () => {
    const source = `
      let f = (items: Array<number>, mut other: Array<number>): void => {
        other.push(2)
      }
    `;
  ...
  it('mixed: immutable param still errors', () => {
    expectErrors(
      `let f = (items: Array<number>, mut other: Array<number>): void => { items.push(1) }`,
      'E240',
    );
  });
});
...
describe('value parameter semantics — Set/Map', () => {
  ...
  it('allows mut Set param.add()', () => {
    expectNoErrors(
      `let f = (mut names: Set<string>): void => { names.add("x") }`,
    );
  });
  ...
  it('allows mut Map param.set()', () => {
    expectNoErrors(
      `let f = (mut scores: Map<string, number>): void => { scores.set("x", 1) }`,
    );
  });
});
...
describe('value parameter semantics — edge cases', () => {
  ...
  it('for loop variable: item.field = x — no error (not a param)', () => {
    expectNoErrors(
      `let f = (items: Array<{ name: string }>): void => {
        for (item in items) {
          item.name = "changed"
        }
      }`,
  ...
  it('catch parameter mutation: catch (e) { e.x = 1 } — no error (not a function param)', () => {
    expectNoErrors(
      `let f = (): void => {
  ...
  it('passing immutable param to mut-param function — no error (type-level)', () => {
    expectNoErrors(
      `let sortHelper = (mut items: Array<number>): void => { items.sort() }
  ...
  it('extension function mut param: items.push(0) — no error', () => {
    expectNoErrors(
      `fun string.doStuff(mut items: Array<number>): void => { items.push(0) }`,
    );
  });
...
describe('value parameter semantics — diagnostic quality', () => {
  ...
  it('E241 includes suggested fix mentioning mut', () => {
    const errors = getErrors(
      `let f = (user: { name: string }): void => { user.name = "Bob" }`,
    );
    const e241 = errors.find(e => e.code === 'E241');
    ...
    expect(e241!.fix!.description).toContain('mut');
  });

  it('E240/E241 do not fire on let mut local variables', () => {
    expectNoErrors(
      `let f = (): void => {
        let mut items: Array<number> = [1, 2, 3]
        items.push(42)
      }`,
    );
  });
