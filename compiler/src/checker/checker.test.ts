...
import type { Type, PrimitiveType, LiteralType } from './types.js';
import type {
...
describe('literal and const types', () => {
  describe('const inference for immutable bindings', () => {
    ...
    it('let mut x = "hello" infers string (widened)', () => {
      const output = expectNoErrors('let mut x = "hello"');
      const decl = output.typedAST.body[0] as LetDeclaration;
    ...
    it('let mut x = 42 infers number (widened)', () => {
      const output = expectNoErrors('let mut x = 42');
      const decl = output.typedAST.body[0] as LetDeclaration;
      const rt = decl.resolvedType;
      expect(rt).toBeDefined();
      expect(rt!.kind).toBe('primitive');
