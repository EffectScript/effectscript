...
describe('types.ts', () => {
  ...
  describe('isAssignableTo', () => {
    ...
    it('function with rest: fixed-arity lambda assignable to rest target', () => {
      // (x: number) => void should be assignable to (x: number, ...rest: number[]) => void
      const source = fnType([param('x', num)], voidT);
      const target: FunctionType = { kind: 'function', params: [param('x', num)], returnType: voidT, rest: { name: 'rest', elementType: num } };
      expect(isAssignableTo(source, target)).toBe(true);
    });

    it('function with rest: rest source assignable to compatible fixed target', () => {
      // (...args: number[]) => void IS assignable to (a: number, b: number) => void
      // because the rest function can accept any number of number args, including exactly 2
      const source: FunctionType = { kind: 'function', params: [], returnType: voidT, rest: { name: 'args', elementType: num } };
      const target = fnType([param('a', num), param('b', num)], voidT);
      expect(isAssignableTo(source, target)).toBe(true);
    });

    it('function with rest: both have rest params, compatible element types', () => {
      const source: FunctionType = { kind: 'function', params: [param('x', num)], returnType: voidT, rest: { name: 'a', elementType: str } };
      const target: FunctionType = { kind: 'function', params: [param('x', num)], returnType: voidT, rest: { name: 'b', elementType: str } };
      expect(isAssignableTo(source, target)).toBe(true);
    });
  ...
  describe('typeToString', () => {
    ...
    it('self-referential union type does not stack overflow', () => {
      // Simulate ReactNode = string | number | Array<ReactNode>
      const reactNode: UnionType = { kind: 'union', members: [] };
      const reactNodeArray: ArrayType = { kind: 'array', element: reactNode };
      (reactNode.members as Type[]).push(str, num, reactNodeArray);
      const result = typeToString(reactNode);
      expect(result).toBeDefined();
      expect(typeof result).toBe('string');
      // Should contain the non-recursive parts
  ...
  describe('flattenUnion', () => {
    ...
    it('self-referential union does not stack overflow', () => {
      // Simulate ReactNode = string | number | ReactNode (self-reference via union member)
      const selfUnion: UnionType = { kind: 'union', members: [] };
      (selfUnion.members as Type[]).push(str, num, selfUnion);
      // flattenUnion should terminate without stack overflow
      const result = flattenUnion([selfUnion]);
      expect(result).toBeDefined();
  ...
  describe('LiteralType', () => {
    ...
    describe('simplifyUnion with literals', () => {
      ...
      // ── Platform type union simplification (TG2) ───────────────
      ...
      it('string! | number! simplifies to (string | number)!', () => {
        const platformStr = makePlatform(str, 'unmappable');
        const platformNum = makePlatform(num, 'unmappable');
        const result = simplifyUnion([platformStr, platformNum]);
        expect(result.kind).toBe('platform');
        if (result.kind === 'platform') {
          expect(result.inner.kind).toBe('union');
          if (result.inner.kind === 'union') {
            expect(result.inner.members).toHaveLength(2);
          }
        }
      });

      it('string! | number produces per-member re-wrapping', () => {
        const platformStr = makePlatform(str, 'unmappable');
        const result = simplifyUnion([platformStr, num]);
        expect(result.kind).toBe('union');
        if (result.kind === 'union') {
          expect(result.members).toHaveLength(2);
          // string member should be platform-wrapped, number should be exact
          const strMember = result.members.find(m =>
            m.kind === 'platform' && m.inner.kind === 'primitive' && m.inner.name === 'string');
          const numMember = result.members.find(m =>
            m.kind === 'primitive' && m.name === 'number');
          expect(strMember).toBeDefined();
          expect(numMember).toBeDefined();
        }
      });
    });
  });
  ...
  describe('platform types', () => {
    ...
    describe('makePlatform', () => {
      it('wraps a simple type', () => {
        const result = makePlatform(str, 'unmappable');
        expect(result.kind).toBe('platform');
        if (result.kind === 'platform') {
          expect(result.inner).toEqual(str);
          expect(result.reason).toBe('unmappable');
        }
      });

      it('normalizes nested platform types (outer reason wins)', () => {
        const inner = makePlatform(str, 'recursive-limit');
        const result = makePlatform(inner, 'budget-cap');
        expect(result.kind).toBe('platform');
        if (result.kind === 'platform') {
          expect(result.inner).toEqual(str);
      ...
      it('collapses ErrorType (returns ErrorType directly)', () => {
        const result = makePlatform(errorT, 'unmappable');
        expect(result.kind).toBe('error');
      });

      it('preserves all reason variants', () => {
        const reasons: PlatformReason[] = ['recursive-limit', 'budget-cap', 'unmappable', 'conditional', 'indexed-access'];
        for (const reason of reasons) {
          const result = makePlatform(num, reason);
          expect(result.kind).toBe('platform');
          if (result.kind === 'platform') {
    ...
    describe('unwrapPlatform', () => {
      it('unwraps platform type to inner + reason', () => {
        const platform = makePlatform(str, 'unmappable');
        const { inner, reason } = unwrapPlatform(platform);
        expect(inner).toEqual(str);
        expect(reason).toBe('unmappable');
      });

      it('passes non-platform through with undefined reason', () => {
    ...
    // ── typesEqual with platform ────────────────────────────────

    describe('typesEqual', () => {
      it('two platform types with same inner are equal (reason ignored)', () => {
        const a = makePlatform(str, 'unmappable');
        const b = makePlatform(str, 'recursive-limit');
        expect(typesEqual(a, b)).toBe(true);
      });

      it('two platform types with different inner are not equal', () => {
        const a = makePlatform(str, 'unmappable');
        const b = makePlatform(num, 'unmappable');
        expect(typesEqual(a, b)).toBe(false);
      });

      it('platform(string) is not equal to plain string', () => {
        const platform = makePlatform(str, 'unmappable');
        expect(typesEqual(platform, str)).toBe(false);
      });
    });

    // ── isAssignableTo with platform ────────────────────────────

    describe('isAssignableTo', () => {
      it('platform(string) assignable to string', () => {
        const platform = makePlatform(str, 'unmappable');
        expect(isAssignableTo(platform, str)).toBe(true);
      });

      it('string assignable to platform(string)', () => {
        const platform = makePlatform(str, 'unmappable');
        expect(isAssignableTo(str, platform)).toBe(true);
      });

      it('platform(string) not assignable to number', () => {
        const platform = makePlatform(str, 'unmappable');
        expect(isAssignableTo(platform, num)).toBe(false);
      });

      it('platform(string) assignable to Any', () => {
        const platform = makePlatform(str, 'unmappable');
        expect(isAssignableTo(platform, anyT)).toBe(true);
      });

      it('Any assignable to platform(string)', () => {
        const platform = makePlatform(str, 'unmappable');
        expect(isAssignableTo(anyT, platform)).toBe(true);
      });

      it('platform(string) assignable to platform(string)', () => {
        const a = makePlatform(str, 'unmappable');
        const b = makePlatform(str, 'recursive-limit');
        expect(isAssignableTo(a, b)).toBe(true);
      });

      it('platform(record) assignable to matching record', () => {
        const rec = record({ name: str });
        const platform = makePlatform(rec, 'unmappable');
        expect(isAssignableTo(platform, rec)).toBe(true);
      });
    });

    // ── typeToString with platform ──────────────────────────────

    describe('typeToString', () => {
      it('renders primitive with ! suffix', () => {
        const platform = makePlatform(str, 'unmappable');
        expect(typeToString(platform)).toBe('string!');
      });

      it('renders number with ! suffix', () => {
        const platform = makePlatform(num, 'unmappable');
        expect(typeToString(platform)).toBe('number!');
      });

      it('parenthesizes function types before ! suffix', () => {
        const fn = fnType([param('x', num)], str);
        const platform = makePlatform(fn, 'unmappable');
        expect(typeToString(platform)).toBe('((number) => string)!');
      });

      it('parenthesizes union types before ! suffix', () => {
        const u = union(str, num);
        const platform = makePlatform(u, 'unmappable');
        expect(typeToString(platform)).toBe('(string | number)!');
      });

      it('record with ! suffix', () => {
        const rec = record({ name: str });
        const platform = makePlatform(rec, 'unmappable');
        expect(typeToString(platform)).toBe('{ name: string }!');
      });

      it('no double ! for nested platform (normalization prevents this)', () => {
        const inner = makePlatform(str, 'recursive-limit');
    ...
    // ── resolveType passthrough ─────────────────────────────────

    describe('resolveType', () => {
      it('passes platform types through unchanged', () => {
        const platform = makePlatform(str, 'unmappable');
        const resolved = resolveType(platform);
        expect(resolved).toBe(platform);
      });
    });

    // ── widenLiteral through platform ───────────────────────────

    describe('widenLiteral', () => {
      it('widens platform(literal) to platform(primitive)', () => {
        const lit = literal('string', 'hello');
        const platform = makePlatform(lit, 'unmappable');
        const result = widenLiteral(platform);
        expect(result.kind).toBe('platform');
        if (result.kind === 'platform') {
          expect(result.inner).toEqual(str);
          expect(result.reason).toBe('unmappable');
      ...
      it('does not widen platform(non-literal)', () => {
        const platform = makePlatform(num, 'unmappable');
        const result = widenLiteral(platform);
    ...
    // ── makeNullable with platform ──────────────────────────────

    describe('makeNullable', () => {
      it('produces platform(nullable(T)) from platform(T) — canonical nesting', () => {
        const platform = makePlatform(str, 'unmappable');
        const result = makeNullable(platform);
        expect(result.kind).toBe('platform');
        if (result.kind === 'platform') {
          expect(result.inner.kind).toBe('nullable');
          if (result.inner.kind === 'nullable') {
            expect(result.inner.inner).toEqual(str);
          }
          expect(result.reason).toBe('unmappable');
        }
      });
