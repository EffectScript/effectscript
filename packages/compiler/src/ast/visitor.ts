...
import type {
  ...
  BinaryExpr, UnaryExpr, CallExpr, NewExpr, MemberExpr, IndexExpr, IfExpr, MatchExpr,
  BlockExpr, ArrowFunction, TryCatchExpr, ArrayExpr, RecordExpr, TemplateString, AwaitExpr,
  ThisExpr, NamedArgument, TupleExpr,
...
export interface ASTVisitor {
  ...
  enterInterfaceDeclaration?(node: InterfaceDeclaration, context: VisitorContext): void;
  leaveInterfaceDeclaration?(node: InterfaceDeclaration, context: VisitorContext): void;
  ...
  enterTupleExpr?(node: TupleExpr, context: VisitorContext): void;
  leaveTupleExpr?(node: TupleExpr, context: VisitorContext): void;
...
function getChildren(node: ASTNodeBase): readonly ASTNodeBase[] {
  ...
  switch (kind) {
    ...
    case 'LetDeclaration': {
      ...
      if (n.tuplePattern) children.push(n.tuplePattern);
    ...
    case 'InterfaceDeclaration': {
      ...
      children.push(n.name);
      if (n.typeParams) for (const tp of n.typeParams) children.push(tp);
      if (n.extends) for (const ext of n.extends) children.push(ext);
      // Reach through property sub-structures to child AST nodes
      for (const prop of n.properties) {
        children.push(prop.typeAnnotation);
      }
      // Reach through method sub-structures to child AST nodes
      for (const method of n.methods) {
        children.push(method.name);
        if (method.typeParams) for (const tp of method.typeParams) children.push(tp);
        for (const param of method.params) {
          children.push(param.name);
          if (param.type) children.push(param.type);
        }
        if (method.returnType) children.push(method.returnType);
      }
      break;
    }
    // ── Expressions ──
    ...
    case 'RecordType': {
      ...
      // IndexSignatureField doesn't extend ASTNodeBase — reach through to valueType
      if (n.indexSignature) {
        children.push(n.indexSignature.valueType);
      }
      break;
    }
    case 'NullableType': {
