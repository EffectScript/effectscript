...
function serializeType(type: unknown): unknown {
  ...
  if (t.kind === 'lazy-record') {
    // Lazy records reference live TS compiler API types and cannot be serialized.
    // Serialize only the already-resolved fields as a regular record.
    const lazy = t as import('../checker/types.js').LazyRecordType;
    return { kind: 'record', fields: mapToRecord(lazy.resolvedFields, serializeType) };
  }
  if (t.kind === 'adt') {
  ...
  if (t.kind === 'function') {
    return {
      ...
      ...(t.rest ? { rest: { name: t.rest.name, elementType: serializeType(t.rest.elementType) } } : {}),
    };
  }
...
function deserializeType(obj: unknown): Type {
  ...
  if (kind === 'function') {
    ...
    if (t['rest']) {
      const rest = t['rest'] as Record<string, unknown>;
      result['rest'] = { name: rest['name'] as string, elementType: deserializeType(rest['elementType']) };
    }
