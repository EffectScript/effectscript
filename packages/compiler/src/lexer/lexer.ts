...
class Lexer {
  ...
  /**
   * Read a numeric literal (decimal, hexadecimal, or bigint).
   *
   * Supports:
   * - Decimal integers: `42`
   * - Decimal floats: `3.14` (requires a digit after the dot)
   * - Hex integers: `0xFF`, `0XAB`
   * - BigInt literals: `42n`, `0xFFn` (integer followed by `n`)
   *
   * Reports `E005` if `0x` is not followed by hex digits.
   * Reports `E006` if a decimal number is followed by `n`.
   */
  private readNumber(): Omit<Token, 'leadingTrivia' | 'trailingTrivia'> {
    ...
    if (this.peek() === '0' && (this.peekAt(1) === 'x' || this.peekAt(1) === 'X')) {
      ...
      if (this.peek() === 'n') {
        ...
        this.column++;
        const text = this.source.slice(startPos, this.pos);
        return { kind: 'BigIntLiteral', text, span: this.makeSpan(start, this.currentPosition()) };
      }
    ...
    if (this.peek() === 'n') {
      if (hasDecimal) {
        // E006: BigInt literals cannot have a decimal part
        const text = this.source.slice(startPos, this.pos + 1);
        this.pos++;
        this.column++;
        const span = this.makeSpan(start, this.currentPosition());
        this.reportDiagnostic(D.E006, 'BigInt literals cannot have a decimal part', span);
        return { kind: 'Error', text, span };
      }
      this.pos++;
      this.column++;
