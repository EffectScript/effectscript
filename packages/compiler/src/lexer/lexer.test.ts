...
describe('Keywords and Identifiers', () => {
  ...
  it('should tokenize `var` as keyword token', () => {
    const tok = first('var');
    expect(tok.kind).toBe('var');
    expect(tok.text).toBe('var');
  });

  it('should tokenize `mut` as identifier (no longer a keyword)', () => {
    const tok = first('mut');
    expect(tok.kind).toBe('Identifier');
    expect(tok.text).toBe('mut');
  });
...
describe('BigInt Literals', () => {
  it('should tokenize 42n as BigIntLiteral', () => {
    const tok = first('42n');
    expect(tok.kind).toBe('BigIntLiteral');
    expect(tok.text).toBe('42n');
  });

  it('should tokenize 0n as BigIntLiteral', () => {
    const tok = first('0n');
    expect(tok.kind).toBe('BigIntLiteral');
    expect(tok.text).toBe('0n');
  });

  it('should tokenize very large bigint literal', () => {
    const tok = first('9007199254740993n');
    expect(tok.kind).toBe('BigIntLiteral');
    expect(tok.text).toBe('9007199254740993n');
  });

  it('should tokenize hex bigint 0xFFn', () => {
    const tok = first('0xFFn');
    expect(tok.kind).toBe('BigIntLiteral');
    expect(tok.text).toBe('0xFFn');
  });

  it('should tokenize hex bigint 0XABn (uppercase X)', () => {
    const tok = first('0XABn');
    expect(tok.kind).toBe('BigIntLiteral');
    expect(tok.text).toBe('0XABn');
  });

  it('should report E006 for decimal bigint 3.14n', () => {
  ...
  it('should tokenize plain 42 as NumberLiteral (regression)', () => {
    const tok = first('42');
    expect(tok.kind).toBe('NumberLiteral');
    expect(tok.text).toBe('42');
  });

  it('should tokenize 42n + 10n as BigIntLiteral Plus BigIntLiteral', () => {
