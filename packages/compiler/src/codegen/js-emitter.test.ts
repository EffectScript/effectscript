...
import type {
  ...
  NumberLiteral, BigIntLiteral, StringLiteral, BooleanLiteral, NullLiteral,
...
function bigint(raw: string): BigIntLiteral {
  return { kind: 'BigIntLiteral', raw, span };
}
  ...
  it('emits nothing for named record type alias', () => {
    const recordType: RecordTypeNode = {
      ...
      fields: [
        ...
        { name: id('email'), type: { kind: 'NamedType', name: id('string'), span } as unknown as RecordTypeField['type'], optional: false, mutable: false },
      ],
      span,
    };
    const node: Record<string, unknown> = {
...
describe('Async/Await emission', () => {
  ...
  describe('for-loop destructuring emission', () => {
    ...
    it('tuple destructuring emits for (const [a, b] of items)', () => {
      const tuplePat: import('../parser/ast.js').TuplePattern = {
        kind: 'TuplePattern',
        elements: [
          { kind: 'BindingPattern' as const, name: id('key'), span },
          { kind: 'BindingPattern' as const, name: id('value'), span },
        ],
        span,
      };
    ...
    it('tuple with wildcard emits for (const [, item] of items)', () => {
      const tuplePat: import('../parser/ast.js').TuplePattern = {
        kind: 'TuplePattern',
        elements: [{ kind: 'WildcardPattern', span }, { kind: 'BindingPattern' as const, name: id('item'), span }],
        span,
      };
  ...
  describe('BigIntLiteral', () => {
    it('BigIntLiteral(42) emits 42n', () => {
      const ast = program(letDecl('x', bigint('42')));
      const js = emitJS(ast);
      expect(js).toContain('const x = 42n');
    });

    it('BigIntLiteral(0xFF) emits 0xFFn', () => {
      const ast = program(letDecl('x', bigint('0xFF')));
      const js = emitJS(ast);
      expect(js).toContain('const x = 0xFFn');
    });

    it('BigIntLiteral(0) emits 0n', () => {
      const ast = program(letDecl('x', bigint('0')));
      const js = emitJS(ast);
      expect(js).toContain('const x = 0n');
    });

    it('bigint arithmetic emits without transformation', () => {
      const expr: BinaryExpr = { kind: 'BinaryExpr', operator: '+', left: id('a'), right: id('b'), span };
      const ast = program(letDecl('c', expr));
      const js = emitJS(ast);
      expect(js).toContain('const c = a + b');
    });

    it('unary negation -42n emits -42n', () => {
      const expr: UnaryExpr = { kind: 'UnaryExpr', operator: '-', operand: bigint('42'), span };
      const ast = program(letDecl('x', expr));
      const js = emitJS(ast);
      expect(js).toContain('const x = -42n');
    });
  });
...
// ── IIFE Wrapping Parentheses ────────────────────────────────────

describe('IIFE wrapping parentheses', () => {
  function asyncArrow(params: FunctionParam[], body: Expression): ArrowFunction {
    const node: Record<string, unknown> = { kind: 'ArrowFunction', async: true, params, body, span };
    return node as unknown as ArrowFunction;
  }
  ...
  it('async IIFE emits (async () => { ... })()', () => {
    // CallExpr whose callee is an async ArrowFunction
    const fn = asyncArrow([], block(
      letDecl('data', awaitExpr(call(id('fetchData'), []))),
      id('data'),
    ));
    const iife = call(fn, []);
    const ast = program(letDecl('result', iife));
    const js = emitJS(ast);
    expect(js).toContain('(async () => {');
    expect(js).toContain('})(');
    expect(js).not.toMatch(/[^(]async \(\) => \{/);
  });

  it('sync IIFE emits (() => { ... })()', () => {
    // CallExpr whose callee is a sync ArrowFunction
    const fn = arrow([], block(
      letDecl('a', num(1)),
      id('a'),
    ));
    const iife = call(fn, []);
  ...
  it('nested IIFE (IIFE inside IIFE) emits correctly', () => {
    const innerFn = arrow([], block(num(42)));
    const innerIife = call(innerFn, []);
    const outerFn = arrow([], block(
      letDecl('x', innerIife),
      id('x'),
    ));
    const outerIife = call(outerFn, []);
    const ast = program(letDecl('result', outerIife));
    const js = emitJS(ast);
    // Both IIFEs should have wrapping parens
    const iifeMatches = js.match(/\(\(\) => \{/g) ?? [];
    expect(iifeMatches.length).toBeGreaterThanOrEqual(2);
  });

  it('IIFE as argument to another function', () => {
    const fn = asyncArrow([], block(
      awaitExpr(call(id('compute'), [])),
    ));
    const iife = call(fn, []);
    const ast = program(exprStmt(call(id('foo'), [iife])));
    const js = emitJS(ast);
    expect(js).toContain('foo((async () => {');
    expect(js).toContain('})()');
  });

  // ── Interface type erasure ─────────────────────────────────

  it('interface declaration emits nothing in JS (type erasure)', () => {
    const namedType: NamedType = { kind: 'NamedType', name: id('string'), span };
    ...
    const ast = program(iface);
    const js = emitJS(ast);
    expect(js.trim()).toBe('');
  });

  it('export interface declaration emits nothing in JS (type erasure)', () => {
    const namedType: NamedType = { kind: 'NamedType', name: id('string'), span };
    const iface: InterfaceDeclaration = {
      kind: 'InterfaceDeclaration',
      name: id('Serializable'),
    ...
    const ast = program(exportDecl);
    const js = emitJS(ast);
    expect(js.trim()).toBe('');
  });

  // ── Index Signatures (codegen) ──────────────────────────────

  it('34. bracket access emits obj["key"]', () => {
    ...
    const js = emitJS(program(stmt));
    expect(js.trim()).toBe('obj["key"];');
  });

  it('35. optional bracket access emits obj?.["key"]', () => {
    const indexExpr: Expression = {
      kind: 'IndexExpr',
      object: id('obj'),
      index: str('key'),
  ...
  it('36. bracket access with variable emits obj[variable]', () => {
    const indexExpr: Expression = {
      kind: 'IndexExpr',
      object: id('obj'),
      index: id('variable'),
    ...
    const js = emitJS(program(stmt));
    expect(js.trim()).toBe('obj[variable];');
  });

  it('36a. assignment to IndexExpr emits config["key"] = "value"', () => {
    ...
    const js = emitJS(program(stmt));
    expect(js.trim()).toBe('config["key"] = "value";');
  });
});
