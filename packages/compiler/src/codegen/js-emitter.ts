...
import type {
  ...
  NumberLiteral, BigIntLiteral, StringLiteral, BooleanLiteral,
  Identifier, BinaryExpr, UnaryExpr, CallExpr, NewExpr,
  MemberExpr, IndexExpr, IfExpr, MatchExpr, BlockExpr,
  ...
  Pattern, TypeNode, TupleExpr, TuplePattern,
} from '../parser/ast.js';
...
function emitTopLevel(ctx: EmitContext, node: Declaration | Statement, prelude: PreludeUsage): void {
  switch (node.kind) {
    ...
    case 'InterfaceDeclaration':
      // Interfaces are type-erased — no JS output
      break;
    case 'ForStatement':
...
function emitLetDeclaration(ctx: EmitContext, node: LetDeclaration, prelude: PreludeUsage, exportPrefix: string): void {
  ...
  // Tuple destructuring: let (a, b) = expr → const [a, b] = expr;
  if (node.tuplePattern !== undefined) {
    ctx.addMapping(node.span);
    ctx.write(`${prefix}${keyword} [`);
    emitTuplePatternBindings(ctx, node.tuplePattern);
    ctx.write('] = ');
...
function emitExportDeclaration(ctx: EmitContext, node: ExportDeclaration, prelude: PreludeUsage): void {
  ...
  if (node.declaration !== undefined) {
    ...
    } else if (node.declaration.kind === 'InterfaceDeclaration') {
      // Interfaces are type-erased — no JS output for exported interfaces
    }
    return;
  }
...
function emitForVariable(ctx: EmitContext, variable: ForStatement['variable']): void {
  if (variable.kind === 'Identifier') {
    ...
    ctx.write(variable.elements.map(el => {
      if (el.kind === 'WildcardPattern') return '';
      if (el.kind === 'BindingPattern') return el.name.name;
      return '';  // unreachable — checker enforces only BindingPattern | WildcardPattern in for-loops
    }).join(', '));
    ctx.write(']');
  }
}
...
function isSimpleEndExpr(expr: Expression): boolean {
  return expr.kind === 'Identifier' || expr.kind === 'NumberLiteral' || expr.kind === 'BigIntLiteral' || expr.kind === 'MemberExpr';
}
...
function emitExpr(ctx: EmitContext, node: Expression, prelude: PreludeUsage): void {
  switch (node.kind) {
    case 'NumberLiteral':
      ...
      break;
    case 'BigIntLiteral':
      ctx.write((node as BigIntLiteral).raw + 'n');
      break;
    case 'StringLiteral':
    ...
    case 'MemberExpr':
      ...
      break;
    case 'IndexExpr':
      emitIndexExpr(ctx, node as IndexExpr, prelude);
      break;
    case 'IfExpr':
    ...
    case 'ArrayExpr':
      ...
      break;
    case 'TupleExpr':
      emitTupleExpr(ctx, node as TupleExpr, prelude);
      break;
    case 'RecordExpr':
...
function isTrivialExpr(expr: Expression): boolean {
  return expr.kind === 'Identifier' || expr.kind === 'NumberLiteral' ||
    expr.kind === 'BigIntLiteral' || expr.kind === 'StringLiteral' ||
    expr.kind === 'BooleanLiteral' || expr.kind === 'NullLiteral';
}
...
function emitCallExpr(ctx: EmitContext, node: CallExpr, prelude: PreludeUsage): void {
  ...
  // Arrow function callees need wrapping parens to form a valid IIFE:
  // `(async () => { ... })()` instead of `async () => { ... }()`
  const needsIIFEWrap = node.callee.kind === 'ArrowFunction';
  if (needsIIFEWrap) ctx.write('(');
...
/** Check if an expression has a resolved tuple type, unwrapping nullable. */
function isTupleTyped(node: Expression): boolean {
  if (!node.resolvedType) return false;
  const resolved = resolveType(node.resolvedType);
  if (resolved.kind === 'tuple') return true;
  if (resolved.kind === 'nullable') return resolveType(resolved.inner).kind === 'tuple';
  return false;
}

function emitMemberExpr(ctx: EmitContext, node: MemberExpr, prelude: PreludeUsage): void {
  // Tuple positional indexing: pair.0 → pair[0]
  if (/^\d+$/.test(node.property.name) && isTupleTyped(node.object)) {
...
/** Emit a bracket access expression: `obj["key"]` or `obj?.[key]`. */
function emitIndexExpr(ctx: EmitContext, node: IndexExpr, prelude: PreludeUsage): void {
  emitExpr(ctx, node.object, prelude);
  ctx.write(node.optional ? '?.[' : '[');
  emitExpr(ctx, node.index, prelude);
  ctx.write(']');
}
...
/** Check if a pattern is a catch-all (wildcard, binding, or tuple of all catch-alls). */
function isCatchAllPattern(pattern: Pattern): boolean {
  ...
  if (pattern.kind === 'TuplePattern') return pattern.elements.every(el => isCatchAllPattern(el));
  return false;
}

// ── SubjectPath — composable index path for nested tuple patterns ──────

/** A base expression with a chain of bracket indices for nested tuple access. */
type SubjectPath = { readonly base: Expression; readonly indices: readonly number[] };

/** Emit a SubjectPath: `base[i0][i1]...` */
function emitSubjectPath(ctx: EmitContext, path: SubjectPath, prelude: PreludeUsage): void {
  emitExpr(ctx, path.base, prelude);
  for (const idx of path.indices) ctx.write(`[${idx}]`);
}

/** Recursively collect condition emitters for a TuplePattern. */
function collectTuplePatternConditions(
  ctx: EmitContext, pattern: TuplePattern, path: SubjectPath,
  prelude: PreludeUsage, out: Array<() => void>,
): void {
  for (let i = 0; i < pattern.elements.length; i++) {
    const el = pattern.elements[i];
    const childPath: SubjectPath = { base: path.base, indices: [...path.indices, i] };
    if (el.kind === 'LiteralPattern') {
      out.push(() => {
        emitSubjectPath(ctx, childPath, prelude);
        ctx.write(' === ');
        emitExpr(ctx, el.literal, prelude);
      });
    } else if (el.kind === 'NullPattern') {
      out.push(() => {
        emitSubjectPath(ctx, childPath, prelude);
        ctx.write(' === null');
      });
    } else if (el.kind === 'TuplePattern') {
      collectTuplePatternConditions(ctx, el, childPath, prelude, out);
    }
    // WildcardPattern, BindingPattern, Identifier — catch-all, no condition
  }
}

/** Recursively emit bindings for a TuplePattern in match context. */
function emitTuplePatternMatchBindings(
  ctx: EmitContext, pattern: TuplePattern, path: SubjectPath, prelude: PreludeUsage,
): void {
  for (let i = 0; i < pattern.elements.length; i++) {
    const el = pattern.elements[i];
    const childPath: SubjectPath = { base: path.base, indices: [...path.indices, i] };
    if (el.kind === 'BindingPattern') {
      ctx.writeIndented(`const ${el.name.name} = `);
      emitSubjectPath(ctx, childPath, prelude);
      ctx.writeLine(';');
    } else if (el.kind === 'Identifier') {
      ctx.writeIndented(`const ${el.name} = `);
      emitSubjectPath(ctx, childPath, prelude);
      ctx.writeLine(';');
    } else if (el.kind === 'TuplePattern') {
      emitTuplePatternMatchBindings(ctx, el, childPath, prelude);
    }
    // WildcardPattern, LiteralPattern, NullPattern — no bindings
  }
}
...
function emitPatternCondition(ctx: EmitContext, pattern: Pattern, subject: Expression, prelude: PreludeUsage, adtType?: ADTType): void {
  switch (pattern.kind) {
    ...
    case 'TuplePattern': {
      ...
      collectTuplePatternConditions(ctx, pattern, { base: subject, indices: [] }, prelude, conditions);
...
function emitPatternBindings(ctx: EmitContext, pattern: Pattern, subject: Expression, prelude: PreludeUsage, adtType?: ADTType): void {
  switch (pattern.kind) {
    ...
    case 'RecordPattern':
      ...
      break;
    case 'TuplePattern':
      emitTuplePatternMatchBindings(ctx, pattern, { base: subject, indices: [] }, prelude);
      break;
    default:
      break;
  }
...
/**
 * Emit tuple pattern bindings for array destructuring.
 * Wildcards become empty slots (JS destructuring holes): `const [, b] = pair;`
 */
function emitTuplePatternBindings(ctx: EmitContext, pattern: TuplePattern): void {
  for (let i = 0; i < pattern.elements.length; i++) {
    if (i > 0) ctx.write(', ');
    const el = pattern.elements[i];
    if (el.kind === 'WildcardPattern') {
      // Empty slot — JS destructuring skip
    } else if (el.kind === 'BindingPattern') {
      ctx.write(el.name.name);
    } else if (el.kind === 'Identifier') {
      ctx.write(el.name);
    }
  }
...
/** Emit a tuple expression as a JS array literal `[a, b, ...]`. */
function emitTupleExpr(ctx: EmitContext, node: TupleExpr, prelude: PreludeUsage): void {
  ctx.write('[');
  for (let i = 0; i < node.elements.length; i++) {
    if (i > 0) ctx.write(', ');
    emitExpr(ctx, node.elements[i], prelude);
