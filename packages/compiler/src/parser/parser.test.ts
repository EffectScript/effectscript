...
import type {
  ...
  NumberLiteral, BigIntLiteral, StringLiteral, BooleanLiteral, NullLiteral, Identifier,
...
describe('Declarations', () => {
  describe('LetDeclaration', () => {
    ...
    it('should parse var y = 0', () => {
      const decl = parseFirst<LetDeclaration>('var y = 0');
      expect(decl.kind).toBe('LetDeclaration');
    ...
    it('should parse var with type annotation', () => {
      const decl = parseFirst<LetDeclaration>('var counter: number = 0');
      expect(decl.kind).toBe('LetDeclaration');
      expect(decl.name.name).toBe('counter');
      expect(decl.mutable).toBe(true);
      expect(decl.typeAnnotation).toBeDefined();
    });

    it('should parse export var', () => {
      const { program } = parseSource('export var x = 1');
      const exportDecl = program.body[0] as ExportDeclaration;
      expect(exportDecl.kind).toBe('ExportDeclaration');
      const inner = exportDecl.declaration as LetDeclaration;
      expect(inner.kind).toBe('LetDeclaration');
      expect(inner.mutable).toBe(true);
      expect(inner.exported).toBe(true);
    });

    it('let mut y = 0 produces parse error (mut is now an identifier)', () => {
      const { diagnostics } = parseSource('let mut y = 0');
      const errors = diagnostics.filter(d => d.severity === 'error');
      expect(errors.length).toBeGreaterThan(0);
    });

    it('let mut = 42 parses as immutable binding named mut', () => {
      const decl = parseFirst<LetDeclaration>('let mut = 42');
      expect(decl.kind).toBe('LetDeclaration');
      expect(decl.name.name).toBe('mut');
      expect(decl.mutable).toBe(false);
    });

    it('var mut = 42 parses as mutable binding named mut', () => {
      const decl = parseFirst<LetDeclaration>('var mut = 42');
      expect(decl.kind).toBe('LetDeclaration');
      expect(decl.name.name).toBe('mut');
      expect(decl.mutable).toBe(true);
    });

    it('var inside block parses correctly', () => {
      const program = parseOk('let f = (): number => { var x = 0\nx = x + 1\nreturn x }');
      const decl = program.body[0] as LetDeclaration;
      const fn = decl.initializer as ArrowFunction;
      const block = fn.body as BlockExpr;
      const varDecl = block.body[0] as LetDeclaration;
      expect(varDecl.kind).toBe('LetDeclaration');
      expect(varDecl.mutable).toBe(true);
      expect(varDecl.name.name).toBe('x');
    });

    it('arrow with var param: (var x: number) => x + 1', () => {
      const program = parseOk('let f = (var x: number): number => x + 1');
      const decl = program.body[0] as LetDeclaration;
      const fn = decl.initializer as ArrowFunction;
...
describe('Statements', () => {
  ...
  it('should parse for loop with tuple destructuring: for ((a, b) in pairs)', () => {
    ...
    expect(tp.elements[0].kind).toBe('BindingPattern');
    expect((tp.elements[0] as BindingPattern).name.name).toBe('a');
    expect(tp.elements[1].kind).toBe('BindingPattern');
    expect((tp.elements[1] as BindingPattern).name.name).toBe('b');
  ...
  it('should parse for loop with wildcard in tuple: for ((_, item) in items.withIndex())', () => {
    ...
    expect(tp.elements[1].kind).toBe('BindingPattern');
    expect((tp.elements[1] as BindingPattern).name.name).toBe('item');
  });
  ...
  it('should parse for loop with tuple pattern and method call: for ((index, item) in items.withIndex())', () => {
    ...
    expect(tp.elements).toHaveLength(2);
    expect(tp.elements[0].kind).toBe('BindingPattern');
    expect((tp.elements[0] as BindingPattern).name.name).toBe('index');
    expect(tp.elements[1].kind).toBe('BindingPattern');
    expect((tp.elements[1] as BindingPattern).name.name).toBe('item');
  ...
  it('should parse member assignment: obj.field = value', () => {
    const program = parseOk('var obj = { x: 0 }\nobj.x = 42');
    const stmt = program.body[1] as AssignmentStatement;
    expect(stmt.kind).toBe('AssignmentStatement');
...
describe('Reserved keyword member access', () => {
  ...
  describe('BigIntLiteral', () => {
    it('should parse let x = 42n with BigIntLiteral initializer', () => {
      const decl = parseFirst<LetDeclaration>('let x = 42n');
      expect(decl.kind).toBe('LetDeclaration');
      expect(decl.initializer.kind).toBe('BigIntLiteral');
      expect((decl.initializer as BigIntLiteral).raw).toBe('42');
    });

    it('should parse let x: bigint = 42n with bigint type annotation', () => {
    ...
    it('should parse let x = -42n as UnaryExpr wrapping BigIntLiteral', () => {
      const decl = parseFirst<LetDeclaration>('let x = -42n');
      const expr = decl.initializer as UnaryExpr;
      expect(expr.kind).toBe('UnaryExpr');
      expect(expr.operator).toBe('-');
      expect(expr.operand.kind).toBe('BigIntLiteral');
      expect((expr.operand as BigIntLiteral).raw).toBe('42');
    });

    it('should parse let x: symbol? with symbol type annotation', () => {
      const decl = parseFirst<LetDeclaration>('let x: symbol? = null');
      expect(decl.typeAnnotation).toBeDefined();
      expect(decl.typeAnnotation!.kind).toBe('NullableType');
    });

    it('should parse 42n + 10n as BinaryExpr with BigIntLiteral operands', () => {
      const expr = parseExpr('42n + 10n') as BinaryExpr;
      expect(expr.kind).toBe('BinaryExpr');
      expect(expr.operator).toBe('+');
      expect(expr.left.kind).toBe('BigIntLiteral');
      expect(expr.right.kind).toBe('BigIntLiteral');
    });

    it('should parse 0xFFn as BigIntLiteral with raw "0xFF"', () => {
      const decl = parseFirst<LetDeclaration>('let x = 0xFFn');
      expect(decl.initializer.kind).toBe('BigIntLiteral');
      expect((decl.initializer as BigIntLiteral).raw).toBe('0xFF');
    });
  });
});
...
describe('InterfaceDeclaration', () => {
  it('should parse basic interface with methods', () => {
    const decl = parseFirst<InterfaceDeclaration>(`
      interface Serializable {
        fun serialize(): string
      }
    `);
    expect(decl.kind).toBe('InterfaceDeclaration');
    expect(decl.name.name).toBe('Serializable');
    expect(decl.methods).toHaveLength(1);
    expect(decl.methods[0].name.name).toBe('serialize');
    expect(decl.methods[0].params).toHaveLength(0);
    expect(decl.methods[0].returnType).toBeDefined();
    expect(decl.methods[0].returnType!.kind).toBe('NamedType');
    expect(decl.properties).toHaveLength(0);
    expect(decl.exported).toBe(false);
  });

  it('should parse interface with properties (let)', () => {
    const decl = parseFirst<InterfaceDeclaration>(`
    ...
    expect(decl.properties[0].name.name).toBe('size');
    expect(decl.properties[0].mutable).toBe(false);
    expect(decl.properties[0].typeAnnotation.kind).toBe('NamedType');
  });

  it('should parse interface with var (mutable) properties', () => {
    const decl = parseFirst<InterfaceDeclaration>(`
      interface Mutable {
        var count: number
      }
    `);
    expect(decl.properties).toHaveLength(1);
    expect(decl.properties[0].name.name).toBe('count');
    expect(decl.properties[0].mutable).toBe(true);
  });
  ...
  it('should parse generic interface', () => {
    ...
    `);
    expect(decl.typeParams).toBeDefined();
    expect(decl.typeParams!).toHaveLength(1);
    expect(decl.typeParams![0].name.name).toBe('T');
    expect(decl.properties).toHaveLength(1);
    expect(decl.methods).toHaveLength(1);
    expect(decl.methods[0].name.name).toBe('map');
    expect(decl.methods[0].typeParams).toBeDefined();
    expect(decl.methods[0].typeParams!).toHaveLength(1);
  });

  it('should parse interface with extends clause (single)', () => {
    const decl = parseFirst<InterfaceDeclaration>(`
      interface ReadableCollection<T> extends Collection<T> {
        fun get(index: number): T
      }
    `);
    expect(decl.extends).toBeDefined();
    expect(decl.extends!).toHaveLength(1);
    expect((decl.extends![0] as NamedType).name.name).toBe('Collection');
  });

  it('should parse interface with extends clause (multiple)', () => {
    const decl = parseFirst<InterfaceDeclaration>(`
      interface Combined extends Foo, Bar<T> {
        fun doSomething(): void
      }
    `);
    expect(decl.extends).toBeDefined();
    expect(decl.extends!).toHaveLength(2);
  });
  ...
  it('should parse empty interface', () => {
    ...
    `);
    expect(decl.kind).toBe('InterfaceDeclaration');
    expect(decl.name.name).toBe('Marker');
  ...
  it('should parse export interface', () => {
    ...
    expect(exp.declaration!.kind).toBe('InterfaceDeclaration');
    const iface = exp.declaration as InterfaceDeclaration;
    expect(iface.exported).toBe(true);
    expect(iface.name.name).toBe('Serializable');
  });

  it('should reject interface method with body (error recovery)', () => {
    const { diagnostics } = parseSource(`
      interface Bad {
        fun serialize(): string => "hello"
      }
    `);
    expect(diagnostics.some(d => d.severity === 'error')).toBe(true);
  });

  it('should parse interface with duplicate member names (parser accepts, checker rejects)', () => {
    const decl = parseFirst<InterfaceDeclaration>(`
      interface Dup {
        let name: string
        fun name(): string
  ...
  it('should parse interface with optional method parameters', () => {
    const decl = parseFirst<InterfaceDeclaration>(`
      interface Opts {
        fun configure(timeout: number, retries: number): void
      }
    `);
    expect(decl.methods[0].params).toHaveLength(2);
    expect(decl.methods[0].params[0].name.name).toBe('timeout');
  });

  it('should produce parse error for let interface = 42 (interface is keyword)', () => {
    const { diagnostics } = parseSource('let interface = 42');
    expect(diagnostics.some(d => d.severity === 'error')).toBe(true);
  });

  it('should produce parse error for let extends = 42 (extends is keyword)', () => {
    const { diagnostics } = parseSource('let extends = 42');
    expect(diagnostics.some(d => d.severity === 'error')).toBe(true);
  });

  it('should parse interface with __call method (parser accepts it)', () => {
    const decl = parseFirst<InterfaceDeclaration>(`
      interface Callable {
        fun __call(msg: string): void
      }
    `);
    expect(decl.methods).toHaveLength(1);
    expect(decl.methods[0].name.name).toBe('__call');
  });

  it('should reject non-named type in extends clause', () => {
    const { diagnostics } = parseSource(`
      interface Bad extends (number) => string {
        fun foo(): void
      }
    `);
    expect(diagnostics.some(d => d.severity === 'error')).toBe(true);
  ...
  it('should parse interface with mixed properties and methods', () => {
    const decl = parseFirst<InterfaceDeclaration>(`
  ...
  it('should reject var parameter in interface method signature', () => {
    const { diagnostics } = parseSource(`
      interface Bad {
        fun process(var input: string): void
      }
    `);
    expect(diagnostics.some(d => d.severity === 'error')).toBe(true);
  });
...
describe('Index Signatures', () => {
  it('1. parse index signature in record type: { [string]: number }', () => {
    const decl = parseFirst<LetDeclaration>('let x: { [string]: number } = getConfig()');
    expect(decl.typeAnnotation).toBeDefined();
    const rt = decl.typeAnnotation as RecordType;
    expect(rt.kind).toBe('RecordType');
    expect(rt.indexSignature).toBeDefined();
    expect(rt.indexSignature!.keyType).toBe('string');
    expect(rt.indexSignature!.valueType.kind).toBe('NamedType');
    expect(rt.fields).toHaveLength(0);
  });

  it('2. parse index signature with number key: { [number]: string }', () => {
    const decl = parseFirst<LetDeclaration>('let x: { [number]: string } = getMap()');
    const rt = decl.typeAnnotation as RecordType;
    expect(rt.indexSignature).toBeDefined();
    expect(rt.indexSignature!.keyType).toBe('number');
  });

  it('3. parse mixed named + index signature: { name: string, [string]: Any }', () => {
    const decl = parseFirst<LetDeclaration>('let x: { name: string, [string]: Any } = getObj()');
    const rt = decl.typeAnnotation as RecordType;
    expect(rt.fields).toHaveLength(1);
    expect(rt.fields[0].name.name).toBe('name');
    expect(rt.indexSignature).toBeDefined();
    expect(rt.indexSignature!.keyType).toBe('string');
  });

  it('4. parse index signature in type declaration', () => {
    const decl = parseFirst<TypeDeclaration>('type Config = { [string]: string }');
    expect(decl.recordType).toBeDefined();
    const rt = decl.recordType as RecordType;
    expect(rt.indexSignature).toBeDefined();
    expect(rt.indexSignature!.keyType).toBe('string');
  });

  it('5. reject invalid key type: { [boolean]: string } reports E120', () => {
    const { diagnostics } = parseSource('let x: { [boolean]: string } = getObj()');
    expect(diagnostics.some(d => d.code === 'E120')).toBe(true);
  });

  it('5a. reject duplicate index signature: { [string]: string, [number]: number } reports E293', () => {
    const { diagnostics } = parseSource('let x: { [string]: string, [number]: number } = getObj()');
    expect(diagnostics.some(d => d.code === 'E293')).toBe(true);
  });

  it('6. parse bracket access expression: obj["key"]', () => {
    const expr = parseExpr('obj["key"]');
    expect(expr.kind).toBe('IndexExpr');
    const ie = expr as import('./ast.js').IndexExpr;
    expect(ie.object.kind).toBe('Identifier');
    expect(ie.index.kind).toBe('StringLiteral');
    expect(ie.optional).toBe(false);
  });

  it('7. parse bracket access with expression: obj[variable]', () => {
    const expr = parseExpr('obj[variable]');
    expect(expr.kind).toBe('IndexExpr');
    const ie = expr as import('./ast.js').IndexExpr;
    expect(ie.index.kind).toBe('Identifier');
  });

  it('8. parse optional bracket access: obj?.["key"]', () => {
    const expr = parseExpr('obj?.["key"]');
    expect(expr.kind).toBe('IndexExpr');
    const ie = expr as import('./ast.js').IndexExpr;
    expect(ie.optional).toBe(true);
  });

  it('9. parse chained bracket access: obj["a"]["b"]', () => {
    const expr = parseExpr('obj["a"]["b"]');
    expect(expr.kind).toBe('IndexExpr');
    const ie = expr as import('./ast.js').IndexExpr;
    expect(ie.object.kind).toBe('IndexExpr');
    const inner = ie.object as import('./ast.js').IndexExpr;
    expect(inner.object.kind).toBe('Identifier');
  });

  it('10. parse bracket access after member access: obj.field["key"]', () => {
    const expr = parseExpr('obj.field["key"]');
    expect(expr.kind).toBe('IndexExpr');
    const ie = expr as import('./ast.js').IndexExpr;
    expect(ie.object.kind).toBe('MemberExpr');
  });

  it('10a. parse bracket access then member access: obj["key"].field', () => {
    const expr = parseExpr('obj["key"].field');
    expect(expr.kind).toBe('MemberExpr');
    const me = expr as MemberExpr;
    expect(me.object.kind).toBe('IndexExpr');
  });

  it('10b. newline before bracket does not parse as IndexExpr', () => {
    const program = parseOk('obj\n["key"]');
    // Should parse as two separate expressions: obj and ["key"]
    expect(program.body.length).toBe(2);
  });

  it('10c. parse assignment to IndexExpr: config["key"] = "value"', () => {
    const stmt = parseFirst<AssignmentStatement>('config["key"] = "value"');
    expect(stmt.kind).toBe('AssignmentStatement');
  ...
  it('11a. parse { name: string } — bare field has mutable: false', () => {
    const decl = parseFirst<LetDeclaration>('let x: { name: string } = { name: "a" }');
    const rt = decl.typeAnnotation as RecordType;
    expect(rt.kind).toBe('RecordType');
    expect(rt.fields).toHaveLength(1);
    expect(rt.fields[0].name.name).toBe('name');
    expect(rt.fields[0].mutable).toBe(false);
  });

  it('11b. parse { var name: string } — var field has mutable: true', () => {
    const decl = parseFirst<LetDeclaration>('let x: { var name: string } = { name: "a" }');
    const rt = decl.typeAnnotation as RecordType;
    expect(rt.kind).toBe('RecordType');
    expect(rt.fields).toHaveLength(1);
    expect(rt.fields[0].name.name).toBe('name');
    expect(rt.fields[0].mutable).toBe(true);
  });

  it('11c. parse { name: string, var score: number } — mixed mutability', () => {
    const decl = parseFirst<LetDeclaration>('let x: { name: string, var score: number } = { name: "a", score: 0 }');
    const rt = decl.typeAnnotation as RecordType;
    expect(rt.kind).toBe('RecordType');
    expect(rt.fields).toHaveLength(2);
    expect(rt.fields[0].name.name).toBe('name');
    expect(rt.fields[0].mutable).toBe(false);
    expect(rt.fields[1].name.name).toBe('score');
    expect(rt.fields[1].mutable).toBe(true);
  });

  it('11d. parse { a: number, b: string, var c: boolean } — three fields with correct mutability', () => {
    const decl = parseFirst<LetDeclaration>('let x: { a: number, b: string, var c: boolean } = { a: 0, b: "", c: true }');
    const rt = decl.typeAnnotation as RecordType;
    expect(rt.fields).toHaveLength(3);
    expect(rt.fields[0].mutable).toBe(false);
    expect(rt.fields[1].mutable).toBe(false);
    expect(rt.fields[2].mutable).toBe(true);
  });

  it('11e. parse { var name?: string } — mutable, optional field', () => {
    const decl = parseFirst<LetDeclaration>('let x: { var name?: string } = { }');
    const rt = decl.typeAnnotation as RecordType;
    expect(rt.fields).toHaveLength(1);
    expect(rt.fields[0].name.name).toBe('name');
    expect(rt.fields[0].mutable).toBe(true);
    expect(rt.fields[0].optional).toBe(true);
  });

  it('11f. parse named record: type User = { name: string, var score: number }', () => {
    const decl = parseFirst<TypeDeclaration>('type User = { name: string, var score: number }');
    expect(decl.recordType).toBeDefined();
    expect(decl.recordType!.fields).toHaveLength(2);
    expect(decl.recordType!.fields[0].name.name).toBe('name');
    expect(decl.recordType!.fields[0].mutable).toBe(false);
    expect(decl.recordType!.fields[1].name.name).toBe('score');
    expect(decl.recordType!.fields[1].mutable).toBe(true);
  });

  it('11g. var in ADT variant field produces parse error', () => {
    // ADT variant parser does not recognize var — it produces a parse error
    const { diagnostics } = parseSource('type T = A(var x: number)');
    const errors = diagnostics.filter(d => d.severity === 'error');
    expect(errors.length).toBeGreaterThan(0);
  });
});
