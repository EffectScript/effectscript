...
import type {
  ...
  RecordPatternField, RecordTypeField, RecordType, IndexSignatureField,
  ForRange,
  ErrorNode,
  InterfaceMethodDeclaration, InterfacePropertyDeclaration,
} from './ast.js';
...
class Parser {
  ...
  private synchronize(): void {
    while (!this.atEnd()) {
      ...
      if (k === 'Semicolon' || k === 'RightBrace' || k === 'let' || k === 'var' || k === 'type' || k === 'import' || k === 'export' || k === 'fun') return;
      this.advance();
    }
  }
  ...
  private topLevel(): (Declaration | Statement) | null {
    switch (this.cur().kind) {
      ...
      case 'var': return this.varDecl(false);
      ...
      case 'interface': return this.interfaceDecl(false);
      case 'async':
  ...
  /**
   * Parse a `let` declaration: `let name [: Type] = expr`.
   *
   * @param exported - `true` if this declaration is wrapped by `export`.
   */
  private letDecl(exported: boolean): Declaration | Statement {
    return this.parseBinding(false, this.advance(), exported);
  }

  /**
   * Parse a `var` declaration: `var name [: Type] = expr`.
   *
   * @param exported - `true` if this declaration is wrapped by `export`.
   */
  private varDecl(exported: boolean): Declaration | Statement {
    return this.parseBinding(true, this.advance(), exported);
  }

  /**
   * Shared parsing logic for `let` and `var` declarations.
   *
   * @param mutable  - `true` for `var`, `false` for `let`.
   * @param leadToken - The consumed `let` or `var` keyword token.
   * @param exported  - `true` if this declaration is wrapped by `export`.
   */
  private parseBinding(mutable: boolean, leadToken: Token, exported: boolean): Declaration | Statement {
    const keyword = mutable ? 'var' : 'let';

    // Tuple destructuring: let (a, b) = expr
    if (this.check('LeftParen')) {
      if (mutable) {
        this.error(D.E100, "Cannot use 'var' with tuple destructuring", this.cur().span);
      }
      return this.parseLetTupleDestructuring(leadToken, exported);
    }

    if (!this.check('Identifier')) {
      this.error(D.E100, `Expected identifier after '${keyword}', found '${this.cur().kind}'`, this.cur().span);
      const sp = this.pos - 1;
    ...
    if (this.atEnd() || this.atStmtEnd()) {
      ...
      return withTrivia<Statement>(
        { kind: 'ExpressionStatement', expression: en as unknown as Expression, span: mergeSpans(leadToken.span, this.prev().span) },
        leadToken, this.prev(),
      );
    }
    ...
    const result: Record<string, unknown> = {
      ...
      initializer: init,
      exported,
      span: mergeSpans(leadToken.span, last.span),
    };
    if (typeAnnotation !== undefined) result['typeAnnotation'] = typeAnnotation;
    return withTrivia(result as unknown as Declaration, leadToken, last);
  }

  /**
   * Parse let tuple destructuring: `let (a, b) [: (T, U)] = expr`.
   *
   * Elements are restricted to Identifier or `_` (wildcard). Produces a
   * LetDeclaration with a synthetic `"_tuple"` name and a `tuplePattern` field.
   */
  private parseLetTupleDestructuring(leadToken: Token, exported: boolean): Declaration | Statement {
    const lp = this.advance(); // consume '('
    const elements: import('./ast.js').Pattern[] = [];
    ...
    while (!this.check('RightParen') && !this.atEnd()) {
      if (this.check('Identifier') && this.cur().text === '_') {
        ...
        const ident = this.id(tok);
        elements.push(ident);
      }
      if (!this.match('Comma')) break;
    }

    const rp = this.expect('RightParen');

    if (elements.length < 2) {
      this.error(D.E103, 'Tuple destructuring requires at least 2 elements', mergeSpans(lp.span, rp.span));
    }

    const tuplePattern: import('./ast.js').TuplePattern = {
      kind: 'TuplePattern',
      elements,
      span: mergeSpans(lp.span, rp.span),
    };

    // Synthetic name placeholder
    const syntheticName: Identifier = { kind: 'Identifier', name: '_tuple', span: lp.span };
   ...
   *
   * Grammar:
   *   InterfaceDeclaration := 'interface' Identifier TypeParams? ExtendsClause? '{' InterfaceMember* '}'
   *   ExtendsClause := 'extends' NamedType (',' NamedType)*
   *   InterfaceMember := InterfaceProperty | InterfaceMethod
   *   InterfaceProperty := ('let' | 'var') Identifier ':' TypeNode
   *   InterfaceMethod := 'fun' Identifier TypeParams? '(' ParamList ')' ':' TypeNode
   */
  private interfaceDecl(exported: boolean): Declaration {
    const ifaceTok = this.advance(); // consume 'interface'
    const name = this.id(this.expect('Identifier'));

    let typeParams: TypeParameter[] | undefined;
    if (this.check('Less')) typeParams = this.parseTypeParams();

    // Parse extends clause: 'extends' NamedType (',' NamedType)*
    let extendsTypes: TypeNode[] | undefined;
    if (this.match('extends')) {
      extendsTypes = [];
      do {
        if (!this.check('Identifier')) {
          this.error(D.E100, "Expected interface name in 'extends' clause", this.cur().span);
          break;
        }
        const extNameTok = this.advance();
        const extName = this.id(extNameTok);
        if (this.check('Less')) {
          // Parse <TypeArg, ...> directly (like type annotation parser)
          this.advance(); // consume '<'
          const ta: TypeNode[] = [];
          while (!this.check('Greater') && !this.atEnd()) {
            ta.push(this.parseType());
            if (!this.match('Comma')) break;
          }
          this.expect('Greater');
          const extNode: Record<string, unknown> = {
            ...
            name: extName,
            typeArgs: ta,
            span: mergeSpans(extNameTok.span, this.prev().span),
          };
          extendsTypes.push(extNode as unknown as TypeNode);
        } else {
          extendsTypes.push({
            kind: 'NamedType',
            name: extName,
            span: extNameTok.span,
    ...
    const methods: InterfaceMethodDeclaration[] = [];
    const properties: InterfacePropertyDeclaration[] = [];

    while (!this.check('RightBrace') && !this.atEnd()) {
      if (this.check('fun')) {
        const funTok = this.advance(); // consume 'fun'
        // Method name can be an identifier or __call-style
        const methodNameTok = this.expect('Identifier');
        const methodName = this.id(methodNameTok);

        let methodTypeParams: TypeParameter[] | undefined;
        if (this.check('Less')) methodTypeParams = this.parseTypeParams();

        this.expect('LeftParen');
        const params = this.parseInterfaceMethodParams();
        ...
        if (this.match('Colon')) {
          ...
          this.error(D.E100, `Expected ':' and return type for interface method '${methodName.name}'`, this.cur().span);
        }

        // Check for body (not allowed in interface methods)
        if (this.check('FatArrow') || this.check('LeftBrace')) {
          this.error(D.E100, 'Interface methods are signatures only — bodies are not allowed', this.cur().span);
          // Recover by skipping the body expression
          if (this.match('FatArrow')) {
            this.expr(); // skip the body expression
          } else if (this.match('LeftBrace')) {
            // Skip to matching brace
            let depth = 1;
            while (depth > 0 && !this.atEnd()) {
              if (this.check('LeftBrace')) depth++;
              if (this.check('RightBrace')) depth--;
              if (depth > 0) this.advance();
            }
            if (this.check('RightBrace')) this.advance();
          }
        }

        const methodSpan = returnType
          ? mergeSpans(funTok.span, returnType.span)
          : mergeSpans(funTok.span, this.prev().span);
        ...
        if (methodTypeParams !== undefined) method['typeParams'] = methodTypeParams;
        if (returnType !== undefined) method['returnType'] = returnType;
        methods.push(method as unknown as InterfaceMethodDeclaration);

      } else if (this.check('let') || this.check('var')) {
        const isMutable = this.check('var');
        const propStart = this.advance(); // consume 'let' or 'var'
        const propNameTok = this.expect('Identifier');
        const propName = this.id(propNameTok);
        this.expect('Colon');
        ...
        properties.push({
          ...
          typeAnnotation,
          mutable: isMutable,
          span: mergeSpans(propStart.span, typeAnnotation.span),
        });

      } else {
        this.error(D.E100, "Expected 'let', 'var', or 'fun' in interface body", this.cur().span);
        this.advance(); // skip unexpected token to recover
      }

      // Eat optional semicolons/newlines between members
      this.eatSemicolon();
    }

    const closeBrace = this.expect('RightBrace');
    ...
    const result: Record<string, unknown> = {
      ...
      properties,
      exported,
      span: mergeSpans(ifaceTok.span, closeBrace.span),
    };
    if (typeParams !== undefined) result['typeParams'] = typeParams;
    if (extendsTypes !== undefined && extendsTypes.length > 0) result['extends'] = extendsTypes;
    return withTrivia(result as unknown as Declaration, ifaceTok, closeBrace);
  }

  /**
   * Parse interface method params: no default values, no mutable params.
   * Similar to parseFnParams but with restrictions.
   */
  private parseInterfaceMethodParams(): FunctionParam[] {
    const params: FunctionParam[] = [];
    while (!this.check('RightParen') && !this.atEnd()) {
      if (this.check('var')) {
        this.error(D.E100, 'Mutable parameters not allowed in interface method signatures', this.cur().span);
      ...
      if (this.match('Colon')) {
        ...
        if (this.check('Equal')) {
          this.error(D.E100, 'Default values not allowed in interface method signatures', this.cur().span);
          this.advance(); // skip =
          this.expr(); // skip the default value
        }
        params.push({ kind: 'FunctionParam', name: nm, type: ty, mutable: false, span: mergeSpans(nt.span, ty.span) });
      } else {
        params.push({ kind: 'FunctionParam', name: nm, mutable: false, span: nt.span });
      }
      if (!this.match('Comma')) break;
    }
    return params;
  ...
  /** Parse an `export` declaration: `export let/var/type ...`, `export { ... }`, or `export { ... } from "mod"`. */
  private exportDecl(): Declaration {
    const exp = this.advance();

    if (this.check('let') || this.check('var')) {
      const decl = this.check('var') ? this.varDecl(true) : this.letDecl(true);
    ...
    if (this.check('interface')) {
      const decl = this.interfaceDecl(true);
  ...
  private parseForTuplePattern(): import('./ast.js').TuplePattern {
    ...
    while (!this.check('RightParen') && !this.atEnd()) {
      if (this.check('Identifier') && this.cur().text === '_') {
        ...
        const tok = this.expect('Identifier');
        const ident = this.id(tok);
        elements.push({ kind: 'BindingPattern', name: ident, span: tok.span });
      }
      if (!this.match('Comma')) break;
    }
  ...
  private expr(minPrec: number = 0): Expression {
    ...
    while (true) {
      ...
      if (this.check('QuestionDot')) {
        this.advance(); // consume ?.
        if (this.check('LeftBracket')) {
          this.advance(); // consume [
          const index = this.expr(0);
          this.expect('RightBracket');
          left = { kind: 'IndexExpr', object: left, index, optional: true, span: mergeSpans(left.span, this.prev().span) } as Expression;
          continue;
        }
        // Existing behavior: member access after ?.
        left = this.memberExprAfterDot(left, true);
        continue;
      }
      if (this.check('LeftBracket') && !this.hasNewline()) {
        this.advance(); // consume [
        const index = this.expr(0);
        this.expect('RightBracket');
        left = { kind: 'IndexExpr', object: left, index, optional: false, span: mergeSpans(left.span, this.prev().span) } as Expression;
        continue;
      }
  ...
  private primary(): Expression {
    ...
    switch (t.kind) {
      ...
      case 'BigIntLiteral': {
        const tok = this.advance();
        // Strip the trailing 'n' for the raw value
        return withTrivia<Expression>(
          { kind: 'BigIntLiteral', raw: tok.text.slice(0, -1), span: tok.span },
          tok, tok,
        );
      }
      case 'SimpleString': {
  ...
  private memberExpr(object: Expression, optional: boolean): MemberExpr {
    ...
    return this.memberExprAfterDot(object, optional);
  }

  /** Parse the property part of a member expression after `.` or `?.` has been consumed. */
  private memberExprAfterDot(object: Expression, optional: boolean): MemberExpr {
    // Accept numeric literal for tuple positional indexing: pair.0, pair.1
    if (this.check('NumberLiteral')) {
      const numTok = this.advance();
      const prop: Identifier = { kind: 'Identifier', name: numTok.text, span: numTok.span };
      return { kind: 'MemberExpr', object, property: prop, optional, span: mergeSpans(object.span, numTok.span) };
    }
  ...
  private blockExpr(): BlockExpr {
    ...
    while (!this.check('RightBrace') && !this.atEnd()) {
      switch (this.cur().kind) {
        ...
        case 'var': body.push(this.varDecl(false)); break;
  ...
  /**
   * Disambiguate `(` — is it an arrow function, a tuple expression, or a parenthesized expression?
   *
   * Uses {@link isArrow} for speculative lookahead. If it's an arrow, delegates
   * to {@link arrowFn}. Otherwise parses the first expression. If a comma follows,
   * the rest are collected into a `TupleExpr`. Otherwise, the expression is returned
   * as a parenthesized (grouped) expression.
   */
  private parenExpr(): Expression {
    ...
    // Check for comma — if present, this is a tuple expression
    if (this.check('Comma')) {
      const elements: Expression[] = [first];
      while (this.match('Comma')) {
        // Trailing comma: if next token is ')', stop without parsing another element
        if (this.check('RightParen')) break;
        elements.push(this.expr());
      }
      if (!this.match('RightParen')) this.error(D.E109, "Expected ')' to close '('", this.cur().span);
      return withTrivia<Expression>(
        { kind: 'TupleExpr', elements, span: mergeSpans(lp.span, this.prev().span) } as unknown as Expression,
        lp, this.prev(),
      );
    }

    // Single expression — grouped/parenthesized
    if (!this.match('RightParen')) this.error(D.E109, "Expected ')' to close '('", this.cur().span);
    // Mark expression as parenthesized so ?? / &&-|| mixing check can skip it
    (first as unknown as Record<string, unknown>)['parenthesized'] = true;
    return first;
  }
  ...
  private isArrow(): boolean {
    ...
    try {
      ...
      while (!this.check('RightParen') && !this.atEnd()) {
        // Skip optional `var` keyword before parameter name
        if (this.check('var')) this.advance();
        if (!this.check('Identifier')) return false;
        this.advance();
        if (this.check('Colon')) { this.advance(); this.skipTy(); }
  ...
  private parsePattern(): Pattern {
    ...
    // Tuple pattern in match: (subpat1, subpat2, ...)
    if (t.kind === 'LeftParen') {
      const lp = this.advance(); // consume '('
      ...
      const rp = this.expect('RightParen');
      if (elements.length < 2) {
        this.error(D.E103, 'Tuple pattern requires at least 2 elements', mergeSpans(lp.span, rp.span));
        return { kind: 'WildcardPattern', span: mergeSpans(lp.span, rp.span) };
      }
      return { kind: 'TuplePattern', elements, span: mergeSpans(lp.span, rp.span) };
    }
  ...
  private recordTy(): TypeNode {
    ...
    let indexSignature: IndexSignatureField | undefined;

    while (!this.check('RightBrace') && !this.atEnd()) {
      if (this.check('LeftBracket')) {
        // Parse index signature: [string]: T or [number]: T
        const start = this.cur().span;
        this.advance(); // consume [
        const keyToken = this.expect('Identifier');
        const keyText = keyToken.text;
        if (keyText !== 'string' && keyText !== 'number') {
          this.error(D.E120, `Invalid index signature key type: expected 'string' or 'number', found '${keyText}'`, keyToken.span);
        }
        this.expect('RightBracket');
        this.expect('Colon');
        const valueType = this.parseType();
        if (indexSignature) {
          this.error(D.E293, `Duplicate index signature: type already has a '${indexSignature.keyType}' index signature`, mergeSpans(start, this.prev().span));
        } else {
          indexSignature = {
            keyType: keyText as 'string' | 'number',
            valueType,
            span: mergeSpans(start, this.prev().span),
          };
        }
      } else {
        // Parse named field — check for `var` prefix (mutable field)
        const mutable = this.match('var');
        const nt = this.expect('Identifier');
        const optional = this.match('Question');
        this.expect('Colon');
        fields.push({ name: this.id(nt), type: this.parseType(), optional, mutable });
      }
      if (!this.match('Comma')) break;
    }
    this.expect('RightBrace');
    return omitUndefined<RecordType>({
      kind: 'RecordType' as const,
      fields,
      span: mergeSpans(lb.span, this.prev().span),
